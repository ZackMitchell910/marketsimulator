...
def _load_baseline_stats() -> Dict[str, Dict[str, float]]:
    ...
    for entry in data:
        ...
        if not symbol:
            continue
        cleaned = {k: float(v) for k, v in entry.items() if k != "symbol"}
        out[symbol.upper()] = cleaned
    return out


_BASELINE_STATS = _load_baseline_stats()
_LOG_ROOT = Path(os.getenv("MARKETTWIN_LOG_DIR", Path.cwd() / "logs"))
_LOG_ROOT.mkdir(parents=True, exist_ok=True)
_SCENARIO_LOG_PATH = _LOG_ROOT / "scenarios.log"
...
class ScenarioService:
    ...
    def __init__(
        ...
        self._baseline_stats: Dict[str, Dict[str, float]] = {
            k: dict(v) for k, v in (baseline_stats or _BASELINE_STATS).items()
        }
    ...
    def run(
        ...
        for ticker, weight in impacts:
            ...
            if len(base.index) >= 2:
                ...
                step_minutes = max(1.0, freq.total_seconds() / 60.0)
            else:
                step_minutes = 1.0
            ...
            base_volume = (
                stats["adv"] / 390 if stats and stats.get("adv") else float(base["volume"].iloc[-1])
            )
    ...
    def _scenario_params(
        ...
        if stats:
            ...
            # Apply modest baseline volatility for symbols without stats.
    ...
    def _log_scenario(self, scenario_text: str, sentiment: float, impacts: List[Dict[str, object]]) -> None:
        ...
        try:
            with _SCENARIO_LOG_PATH.open("a", encoding="utf-8") as handle:
